...
const unifiedScript = `
-- =================================================================
-- السكربت الشامل لإصلاح وإعداد قاعدة البيانات مع نظام المساعدين والصلاحيات
-- =================================================================
...
-- إذا كان المستخدم محامياً، يعود الـ ID الخاص به.
-- إذا كان مساعداً، يعود الـ ID الخاص بالمحامي الذي يعمل معه.
...
RETURNS void AS $$
BEGIN
    -- السماح للمدير بحذف أي مستخدم، والسماح للمحامي بحذف مساعديه
...
-- دالة التعامل مع المستخدم الجديد (محدثة لدعم المساعدين)
...
BEGIN
    ...
    -- البحث عن المحامي إذا كان المسجل مساعداً
    ...
    VALUES (
      ...
      false,
      found_lawyer_id,
      CASE WHEN found_lawyer_id IS NOT NULL THEN false ELSE false END -- المساعد يحتاج موافقة المحامي
    )
...
-- 3. إنشاء الجداول الأخرى (مع التأكد من وجود الأعمدة)
...
-- 4. سياسات الأمان (RLS Policies) - تحديث شامل لدعم المساعدين
-- نقوم بحذف السياسات القديمة وإنشاء سياسات تعتمد على get_data_owner_id()
...
-- المستخدم يرى ملفه، والمحامي يرى ملفات مساعديه، والمدير يرى الجميع
CREATE POLICY "Profiles Visibility" ON public.profiles FOR SELECT USING (
    auth.uid() = id OR lawyer_id = auth.uid() OR public.is_admin()
);
CREATE POLICY "Profiles Self Update" ON public.profiles FOR UPDATE USING (auth.uid() = id OR lawyer_id = auth.uid());

-- سياسة موحدة لباقي الجداول: الوصول مسموح إذا كان الـ user_id هو نفسه "مالك البيانات" للمستخدم الحالي
-- هذا يعني: المحامي يرى بياناته، والمساعد يرى بيانات محاميه
...
CREATE POLICY "Access Own Data" ON public.cases FOR ALL USING (user_id = public.get_data_owner_id());
CREATE POLICY "Access Own Data" ON public.stages FOR ALL USING (user_id = public.get_data_owner_id());
CREATE POLICY "Access Own Data" ON public.sessions FOR ALL USING (user_id = public.get_data_owner_id());
CREATE POLICY "Access Own Data" ON public.admin_tasks FOR ALL USING (user_id = public.get_data_owner_id());
CREATE POLICY "Access Own Data" ON public.appointments FOR ALL USING (user_id = public.get_data_owner_id());
CREATE POLICY "Access Own Data" ON public.accounting_entries FOR ALL USING (user_id = public.get_data_owner_id());
CREATE POLICY "Access Own Data" ON public.invoices FOR ALL USING (user_id = public.get_data_owner_id());
CREATE POLICY "Access Own Data" ON public.invoice_items FOR ALL USING (user_id = public.get_data_owner_id());
CREATE POLICY "Access Own Data" ON public.case_documents FOR ALL USING (user_id = public.get_data_owner_id());
...
-- 7. Backfill (إصلاح البيانات القديمة)
        ...
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-[200]">
            <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col">
                <div className="flex items-center gap-3 mb-4 text-amber-600">
                    ...
                    <h2 className="text-2xl font-bold">تحديث قاعدة البيانات (نظام المساعدين)</h2>
                </div>
                        ...
                        <div className="flex">
                            ...
                            <div className="ms-3">
                                <p className="text-sm text-blue-700">
                                    هذا التحديث ضروري لتفعيل نظام المساعدين والصلاحيات. يرجى نسخ الكود الجديد وتشغيله في Supabase لتحديث الجداول والسياسات الأمنية.
                                </p>
                            </div>
                        </div>
                    </div>
