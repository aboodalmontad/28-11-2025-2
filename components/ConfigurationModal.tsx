...
-- 3. إنشاء الجداول
CREATE TABLE IF NOT EXISTS public.assistants (id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, user_id uuid NOT NULL, name text NOT NULL, updated_at timestamptz DEFAULT now(), CONSTRAINT assistants_user_id_name_key UNIQUE (user_id, name));
ALTER TABLE public.assistants ADD COLUMN IF NOT EXISTS updated_at timestamptz DEFAULT now();
DO $$ 
BEGIN 
...
ALTER TABLE public.cases DROP CONSTRAINT IF EXISTS cases_client_id_fkey, ADD CONSTRAINT cases_client_id_fkey FOREIGN KEY (client_id) REFERENCES public.clients(id) ON DELETE CASCADE;
...
ALTER TABLE public.stages DROP CONSTRAINT IF EXISTS stages_case_id_fkey, ADD CONSTRAINT stages_case_id_fkey FOREIGN KEY (case_id) REFERENCES public.cases(id) ON DELETE CASCADE;
...
ALTER TABLE public.sessions DROP CONSTRAINT IF EXISTS sessions_stage_id_fkey, ADD CONSTRAINT sessions_stage_id_fkey FOREIGN KEY (stage_id) REFERENCES public.stages(id) ON DELETE CASCADE;
...
CREATE TABLE IF NOT EXISTS public.invoices (id text NOT NULL PRIMARY KEY, user_id uuid NOT NULL, client_id text NOT NULL REFERENCES public.clients(id) ON DELETE CASCADE, client_name text, case_id text, case_subject text, issue_date timestamptz NOT NULL, due_date timestamptz NOT NULL, tax_rate real DEFAULT 0, discount real DEFAULT 0, status text DEFAULT 'draft', notes text, updated_at timestamptz DEFAULT now());
ALTER TABLE public.invoices DROP CONSTRAINT IF EXISTS invoices_client_id_fkey, ADD CONSTRAINT invoices_client_id_fkey FOREIGN KEY (client_id) REFERENCES public.clients(id) ON DELETE CASCADE;
...
CREATE TABLE IF NOT EXISTS public.case_documents (id text NOT NULL PRIMARY KEY, user_id uuid NOT NULL, case_id text NOT NULL REFERENCES public.cases(id) ON DELETE CASCADE, name text NOT NULL, type text NOT NULL, size real NOT NULL, added_at timestamptz DEFAULT now() NOT NULL, storage_path text, updated_at timestamptz DEFAULT now());
ALTER TABLE public.case_documents DROP CONSTRAINT IF EXISTS case_documents_case_id_fkey, ADD CONSTRAINT case_documents_case_id_fkey FOREIGN KEY (case_id) REFERENCES public.cases(id) ON DELETE CASCADE;
...
-- 4. وظائف تتبع الحذف (Deletion Tracking)
CREATE OR REPLACE FUNCTION public.record_deletion()
RETURNS TRIGGER AS $$
BEGIN
    INSERT INTO public.sync_deletions (table_name, record_id, user_id)
    VALUES (TG_TABLE_NAME, OLD.id::text, public.get_data_owner_id());
    RETURN OLD;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER SET search_path = public;

CREATE OR REPLACE FUNCTION public.record_assistant_deletion()
RETURNS TRIGGER AS $$
BEGIN
    INSERT INTO public.sync_deletions (table_name, record_id, user_id)
    VALUES (TG_TABLE_NAME, OLD.name, public.get_data_owner_id());
    RETURN OLD;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER SET search_path = public;
...
DECLARE
    t text;
    tables text[] := ARRAY['clients', 'cases', 'stages', 'sessions', 'admin_tasks', 'appointments', 'accounting_entries', 'invoices', 'invoice_items', 'case_documents', 'site_finances'];
BEGIN
    FOR t IN SELECT unnest(tables) LOOP
        EXECUTE format('DROP TRIGGER IF EXISTS tr_record_deletion ON public.%I', t);
        EXECUTE format('CREATE TRIGGER tr_record_deletion AFTER DELETE ON public.%I FOR EACH ROW EXECUTE PROCEDURE public.record_deletion()', t);
    END LOOP;
    
    -- Special case for assistants
    DROP TRIGGER IF EXISTS tr_record_assistant_deletion ON public.assistants;
    CREATE TRIGGER tr_record_assistant_deletion AFTER DELETE ON public.assistants FOR EACH ROW EXECUTE PROCEDURE public.record_assistant_deletion();
END $$;
