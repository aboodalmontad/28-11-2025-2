...
const App: React.FC<AppProps> = ({ onRefresh }) => {
    ...
    // Pass session and isAuthLoading to hook
    const data = useSupabaseData(session?.user ?? null, isAuthLoading);

    React.useEffect(() => {
        if (session) {
            // Priority 1: Current session user profile in data.profiles
            const userProfile = data.profiles.find(p => p.id === session.user.id);
            if (userProfile) {
                setProfile(userProfile);
            } else if (isOnline && !profile) {
                 // The hook useSupabaseData already tries to fetch the profile.
                 // We don't want to reset it if it's already set.
            }
        } else {
            setProfile(null);
        }
    ...
    }, [session, data.profiles, data.unpostponedSessions, data.setShowUnpostponedSessionsModal, isOnline, profile]);
    ...
    React.useEffect(() => {
        ...
        if (justUpdated === 'true') {
            data.addRealtimeAlert('تم تحديث التطبيق بنجاح إلى الإصدار الجديد (15-2-2026-3)', 'sync');
    ...
    const handleOpenAdminTaskModal = (initialData: any = null) => { setInitialAdminTaskData(initialData); setIsAdminTaskModalOpen(true); };
    ...
    const handleSaveAdminTask = (taskData: Omit<AdminTask, 'completed'> & { id?: string }) => {
        if (taskData.id) {
            ...
            const maxOrderIndex = data.adminTasks.filter(t => (t.location || 'غير محدد') === newLocation).reduce((max, t) => Math.max(max, t.orderIndex || 0), -1);
            const newTask: AdminTask = { id: `task-${Date.now()}`, ...restOfTaskData, completed: false, orderIndex: maxOrderIndex + 1, updated_at: new Date() };
    ...
    const handleCreateInvoice = (clientId: string, caseId?: string) => { setInitialInvoiceData({ clientId, caseId }); setCurrentPage('accounting'); };
    ...
    const handleShareAssigneeReport = (assignee: string | null) => {
        ...
        if (filteredAppointments.length > 0) {
            ...
            filteredAppointments.forEach(a => { message += `- (${formatTime(a.time)}) ${a.title}`; if (a.importance !== 'normal') message += ` (${a.importance})`; message += `\n`; });
            message += `\n`;
        }
        ...
        if (taskLocations.length > 0) {
            ...
            taskLocations.forEach(location => {
                ...
                if (tasks.length > 0) {
                    ...
                    tasks.forEach(t => { message += `- ${t.task} - تاريخ الاستحقاق: ${formatDate(t.dueDate)}\n`; });
                }
            });
        }
    ...
    // Safety check for effective Profile
    const effectiveProfile = profile || data.profiles.find(p => p.id === session.user.id);
    
    if (!effectiveProfile) {
         if (isOnline) {
             return <FullScreenLoader text="جاري تحميل الملف الشخصي من السحابة..." />;
         }
    ...
    if (effectiveProfile && !effectiveProfile.mobile_verified && effectiveProfile.role !== 'admin') {
         return (
            <React.Suspense fallback={<FullScreenLoader />}>
                <LoginPage onForceSetup={() => setShowConfigModal(true)} onLoginSuccess={(u) => setSession({user: u} as AuthSession)} initialMode="otp" currentUser={session.user} currentMobile={effectiveProfile.mobile_number} onLogout={handleLogout} onVerificationSuccess={data.fetchAndRefresh} />
            </React.Suspense>
         );
    }
    ...
    const renderPage = () => {
        switch (currentPage) {
            ...
            case 'home':
            default:
                if (!data.permissions.can_view_agenda) return <div className="flex flex-col items-center justify-center h-full text-center text-gray-500"> <ExclamationCircleIcon className="w-16 h-16 text-gray-300 mb-4" /> <p className="text-lg font-semibold">ليس لديك صلاحية لعرض المفكرة.</p> <p className="text-sm">يرجى التواصل مع المحامي لتحديث الصلاحيات.</p> </div>;
    ...
    const homePageActions = (
        <div ref={actionsMenuRef} className="relative">
            <button onClick={() => setIsActionsMenuOpen(prev => !prev)} className="p-2 text-gray-600 rounded-full hover:bg-gray-100 transition-colors" aria-label="إجراءات جدول الأعمال"> <PrintIcon className="w-5 h-5" /> </button>
            {isActionsMenuOpen && (
        ...
        <DataProvider value={data}>
            <div className="flex flex-col h-screen bg-gray-50">
                ...
                <main className="flex-grow p-4 sm:p-6 overflow-y-auto pb-20 sm:pb-6">
                    <React.Suspense fallback={<FullScreenLoader />}> {renderPage()} </React.Suspense>
                </main>
                ...
                {isPrintAssigneeModalOpen && (
                    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-start justify-center z-50 no-print p-4 overflow-y-auto" onClick={() => setIsPrintAssigneeModalOpen(false)}>
                        <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-lg" onClick={e => e.stopPropagation()}>
                            <h2 className="text-xl font-bold mb-4 border-b pb-3">اختر الشخص للطباعة</h2>
                            <div className="space-y-3 max-h-80 overflow-y-auto">
                                <button onClick={() => handleGenerateAssigneeReport(null)} className="w-full text-right px-4 py-3 bg-blue-50 text-blue-800 font-semibold rounded-lg hover:bg-blue-100">جدول الأعمال العام</button>
                                {data.assistants.map(name => ( <button key={name} onClick={() => handleGenerateAssigneeReport(name)} className="w-full text-right block px-4 py-2 bg-gray-50 rounded-md hover:bg-gray-100">{name}</button> ))}
                            </div>
                        </div>
                    </div>
                )}
                    ...
                    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" onClick={() => setIsPrintModalOpen(false)}>
                        <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
                            ...
                            <div className="mt-6 flex justify-end gap-4 border-t pt-4 no-print">
                                <button className="px-6 py-2 bg-gray-200 rounded-lg" onClick={() => setIsPrintModalOpen(false)}>إغلاق</button>
                                <button className="flex items-center gap-2 px-6 py-2 bg-blue-600 text-white rounded-lg" onClick={() => printElement(printReportRef.current)}> <PrintIcon className="w-5 h-5" /> <span>طباعة</span> </button>
                            </div>
                        </div>
                    </div>
                )}
