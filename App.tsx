    ...
    return (
        <header className="bg-white shadow-md p-2 sm:p-4 flex justify-between items-center no-print sticky top-0 z-30">
            <div className="flex items-center gap-4">
                <h1 className="text-lg font-bold text-gray-800 hidden sm:block">مكتب المحامي</h1>
                <div className="flex items-center gap-2 px-3 py-1 bg-blue-50 rounded-full border border-blue-100">
                    <UserIcon className="w-4 h-4 text-blue-600" />
                    <span className="text-xs font-bold text-blue-800 truncate max-w-[120px]">{userName}</span>
                </div>
                 <div className="hidden md:flex items-center gap-1">
                    {navItems.map(item => (
                        <button key={item.id} onClick={() => onNavigate(item.id as Page)} className={`px-3 py-2 rounded-md text-sm font-medium transition-colors ${currentPage === item.id ? 'bg-blue-100 text-blue-700' : 'text-gray-600 hover:bg-gray-100'}`}>
                            {item.label}
                        </button>
                    ))}
                </div>
            ...
            <div className="flex items-center gap-2">
                ...
                />
                <button onClick={() => onNavigate('settings')} className="p-2 rounded-full text-gray-500 hover:bg-gray-100"><Cog6ToothIcon className="w-5 h-5" /></button>
                <button onClick={onLogout} className="p-2 text-red-500 hover:bg-red-50 rounded-full"><PowerIcon className="w-5 h-5" /></button>
            </div>
        </header>
    );
};
    ...
    useEffect(() => {
        ...
        const { data: { subscription } } = supabase.auth.onAuthStateChange((event, newSession) => {
            ...
            if (newSession) {
                ...
                localStorage.setItem('lawyerAppLastUser', JSON.stringify(newSession.user));
            } else {
                setSession(null);
        ...
        <DataProvider value={data}>
            <div className="flex flex-col h-screen print:h-auto bg-gray-50 print:bg-white">
                ...
                <main className="flex-grow p-4 overflow-y-auto print:overflow-visible print:p-0 pb-24 md:pb-4 print:pb-0">
                    ...
                    {currentPage === 'accounting' && <AccountingPage clear_initial_invoice_data={()=>{}} />}
