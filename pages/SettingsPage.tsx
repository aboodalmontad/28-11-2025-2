...
import { get_db, DATA_STORE_NAME, DOCS_FILES_STORE_NAME, DOCS_METADATA_STORE_NAME } from '../utils/db';
...
const SettingsPage: React.FC<SettingsPageProps> = () => {
    ...
    const [is_delete_assistant_modal_open, set_is_delete_assistant_modal_open] = React.useState(false);
    const [assistant_to_delete, set_assistant_to_delete] = React.useState<string | null>(null);
    const [new_assistant, set_new_assistant] = React.useState('');
    const [db_stats, set_db_stats] = React.useState<string | null>(null);
    const [is_assistants_manager_open, set_is_assistants_manager_open] = React.useState(false);
    ...
    const handle_confirm_clear_data = () => {
        try {
            const emptyData = { clients: [], admin_tasks: [], appointments: [], accounting_entries: [], assistants: ['بدون تخصيص'] };
            set_full_data(emptyData);
            show_feedback('تم مسح جميع البيانات بنجاح.', 'success');
        } catch (error) { show_feedback('حدث خطأ أثناء مسح البيانات.', 'error'); }
        set_is_confirm_modal_open(false);
    };
    const handle_export_data = () => { if (export_data()) { show_feedback('تم تصدير البيانات بنجاح.', 'success'); } else { show_feedback('فشل تصدير البيانات.', 'error'); } };
    const handle_import_data = (event: React.ChangeEvent<HTMLInputElement>) => {
        const file = event.target.files?.[0]; if (!file) return;
        const reader = new FileReader();
        reader.onload = (e) => { try { const text = e.target?.result; if (typeof text !== 'string') throw new Error("File could not be read."); const data = JSON.parse(text); set_full_data(data); show_feedback('تم استيراد البيانات بنجاح.', 'success'); } catch (error) { show_feedback('فشل استيراد البيانات.', 'error'); } };
    ...
    const handle_inspect_db = async () => { 
        ...
        try { 
            ...
            let stats = ''; 
            const stores = [DATA_STORE_NAME, DOCS_METADATA_STORE_NAME, DOCS_FILES_STORE_NAME]; 
    ...
    const ToggleSwitch: React.FC<{ enabled: boolean; on_change: (enabled: boolean) => void; label: string }> = ({ enabled, on_change, label }) => (
        <div className="flex items-center">
            ...
            <button type="button" className={`${enabled ? 'bg-blue-600' : 'bg-gray-200'} relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none`} role="switch" aria-checked={enabled} onClick={() => on_change(!enabled)}>
        ...
        <div className="space-y-6">
            ...
            {permissions?.can_delete_client && (
                <div className="bg-white p-6 rounded-lg shadow space-y-4">
                    ...
                    <button onClick={() => set_is_assistants_manager_open(true)} className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white font-semibold rounded-lg hover:bg-blue-700 transition-colors"><UserGroupIcon className="w-5 h-5" /><span>فتح لوحة تعريف المساعدين</span></button>
                </div>
            )}

            <div className="bg-white p-6 rounded-lg shadow space-y-4">
                <h2 className="text-xl font-bold text-gray-800 border-b pb-3">إعدادات المزامنة</h2>
                <div className="pt-2"><ToggleSwitch label="المزامنة التلقائية" enabled={is_auto_sync_enabled} on_change={set_auto_sync_enabled} /></div>
            </div>
            <div className="bg-white p-6 rounded-lg shadow space-y-4">
                <h2 className="text-xl font-bold text-gray-800 border-b pb-3">النسخ الاحتياطي</h2>
                <div className="pt-2"><ToggleSwitch label="النسخ الاحتياطي اليومي التلقائي" enabled={is_auto_backup_enabled} on_change={set_auto_backup_enabled} /></div>
            </div>
    ...
    return <li key={name} className="flex justify-between p-2 bg-gray-50 border rounded">{name}{name !== 'بدون تخصيص' && <button onClick={() => handle_delete_assistant(name)}><TrashIcon className="w-4 h-4 text-red-500"/></button>}</li>;
            ...
            {is_delete_assistant_modal_open && <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50"><div className="bg-white p-8 rounded shadow-lg"><p className="mb-4">حذف المساعد؟</p><div className="flex gap-4"><button onClick={() => set_is_delete_assistant_modal_open(false)} className="px-4 py-2 bg-gray-200 rounded">إلغاء</button><button onClick={handle_confirm_delete_assistant} className="px-4 py-2 bg-red-600 text-white rounded">نعم</button></div></div></div>}
            
            {is_assistants_manager_open && <AssistantsManager onClose={() => set_is_assistants_manager_open(false)} />}
        </div>
    );
};
