...
import { Session, AdminTask, Appointment, Stage, Client } from '../types.ts';
import { formatDate, isSameDay, isBeforeToday, toInputDateString } from '../utils/dateUtils.ts';
import { PrintIcon, PlusIcon, PencilIcon, TrashIcon, SearchIcon, ExclamationTriangleIcon, CalendarIcon, ChevronLeftIcon, ScaleIcon, BuildingLibraryIcon, ShareIcon, UserIcon, ClipboardDocumentIcon, ClipboardDocumentCheckIcon, HomeIcon, ListBulletIcon, ViewColumnsIcon } from '../components/icons.tsx';
                         ...
                         <div className="flex flex-wrap justify-center gap-2">
                            <div className="relative">
                                <button
                                    ...
                                    className={`flex items-center justify-center gap-1 px-3 py-1.5 text-white rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-opacity-50 text-xs font-semibold ${viewMode === 'unpostponed' ? 'bg-red-700' : 'bg-red-600 hover:bg-red-700'}`}
                                >
                                    <ExclamationTriangleIcon className="w-4 h-4" />
                                    <span>غير المرحلة</span>
                                </button>
                                {unpostponedSessions.length > 0 && (
                                     <span className="absolute -top-1 -start-1 flex h-4 w-4 items-center justify-center rounded-full bg-yellow-400 text-black text-xs font-bold ring-1 ring-white animate-pulse" title={`${unpostponedSessions.length} جلسات غير مرحلة`}>
                            ...
                            <button
                                ...
                                className={`flex items-center justify-center gap-1 px-3 py-1.5 text-white rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 text-xs font-semibold ${viewMode === 'daily' ? 'bg-blue-700' : 'bg-blue-600 hover:bg-blue-700'}`}
                            >
                            ...
                            <button
                                ...
                                className={`flex items-center justify-center gap-1 px-3 py-1.5 text-white rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-opacity-50 text-xs font-semibold ${viewMode === 'upcoming' ? 'bg-green-700' : 'bg-green-600 hover:bg-green-700'}`}
                            >
