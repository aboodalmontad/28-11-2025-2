        ...
        downloadQueueRef.current = downloadQueueRef.current.then(async () => {
            ...
            for (const doc of pendingDocs) {
                ...
                try {
                    ...
                    const isNetworkError = String(e?.message || '').includes('Failed to fetch') || String(e?.message || '').toLowerCase().includes('network');
                    if (isNetworkError) {
    ...
    React.useEffect(() => {
        ...
        const loadData = async () => {
            try {
                ...
                if (isOnlineNow) {
                    manualSync().catch(e => {
                        console.error("Initial manualSync failed:", e);
    ...
    return {
        ...
        addDocuments: async (caseId: string, files: FileList) => {
             ...
             for (let i = 0; i < files.length; i++) {
                 ...
                 const doc: CaseDocument = { id: docId, caseId, userId: ownerId, name: file.name, type: file.type || 'application/octet-stream', size: file.size, addedAt: new Date(), storagePath: safeStoragePath, localState: 'pending_upload', updated_at: new Date() };
                 await db.put(DOCS_FILES_STORE_NAME, file, doc.id);
