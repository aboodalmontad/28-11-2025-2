 ...
 * Fix: If it's a midnight UTC string from DB, we treat it as local midnight.
 */
const ensureDate = (date: Date | string | number | undefined | null): Date => {
    if (!date) return new Date();
    if (date instanceof Date) return date;
    
    // If it's a string representing exactly midnight UTC (common in DBs)
 ...
 * Fix: Uses local time components to prevent UTC offset bugs.
 */
...
export const parseInputDateString = (dateString: string | null | undefined): Date | null => {
    ...
    // T00:00:00 ensures it's parsed as local time midnight
